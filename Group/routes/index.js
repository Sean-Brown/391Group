...
exports.ta = function(req, res) {
	...
	res.render('travelaide', {title: 'TravelAide', message: 'Welcome, Please Log In', cvr: 'cov', user:''});    
    }
    else {
	res.render('travelaide', {title: 'TravelAide', message: 'Welcome', cvr: 'nocov', user: req.session.user});
    }
};
...
exports.create = function(req, res) {
    ...
    createNewAccount(req.body.uname, req.body.upass, function(err, obj) {
        if (err) {
            ...
            res.render('travelaide', {title: 'TravelAide Guest', message: 'Unable To Create Account', cvr: 'cov', user: ''});
        }
        else {
...
exports.login = function(req, res) {
	...
	if (result.rows.length === 0) {
	    res.render('travelaide', {title: 'TravelAide Guest', message: 'Welcome, Please Log In',cvr:"cov",user: ''});
	}
	else {
...
exports.removeCountry = function (req, res) {
    deleteCountry(req.session.user, req.body.country,function(err, result) {
	if (err) {
	    console.log("Error removing country "+req.country+" for user "+req.session.user);
	    res.contentType('application/json');
	    res.send({'Error':'Error removing '+req.body.country+', try again later'});
	}
	else {
	    res.contentType('application/json');
	    res.send({'Success':'Successfully removed '+req.body.country+' for '+req.session.user});
	}
    });
};
